...
export async function wallpaperBonus(){
  ...
  return `<section id="tapety" class="platform-section"><p class="kicker">DODATEK / TAPETY 4K</p><h2>Trochę nieba na co dzień.</h2><p class="section-lede">Trzy motywy, każdy w wersji na komputer i telefon. Sześć spokojnych kompozycji bez napisów, z miejscem na ikony i zegar.</p><div class="download-row"><a class="button" href="${manifest.archive.file}" download>Wszystkie tapety 4K · ZIP <span>↓</span></a><button class="copy-post" data-target="wallpaper-share-link">Kopiuj link do tapet</button></div><p id="wallpaper-share-link" class="share-address" tabindex="0">https://jakiesluchawki.github.io/chmurnik/premiera/astra/#tapety</p><p class="hint">Pobierz PNG w pełnej rozdzielczości, nie podgląd. Tapety są też w pełnym pakiecie powyżej. Link możesz wysłać prywatnie, ale sama strona jest dostępna bez logowania.</p><div class="wallpaper-grid">${cards.join('')}</div><p class="hint">Do osobistego użytku. Wymiary dotyczą gotowych eksportów 4K. Telefon może delikatnie przyciąć obraz do proporcji ekranu. <a href="tapety/CZYTAJ-MNIE.txt">Instrukcja i szczegóły</a>.</p></section>`;
}
