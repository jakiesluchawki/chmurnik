...
const zipSize=(await stat(path.join(site,zipName))).size;
const cards=records.map(r=>`<article id="${r.id}"><a class="image-link" href="${r.file}" download><img loading="lazy" width="432" height="768" src="${r.preview}" alt="${escape(r.alt)}"></a><div class="card-copy"><span class="number">${String(r.number).padStart(2,'0')} / 10</span><h2>${escape(r.lead)}</h2><p>${escape(r.body)}</p><div class="actions"><a class="button" href="${r.file}" download>Pobierz PNG <span>↓</span></a><button class="share" data-file="${r.file}">Udostępnij PNG</button></div><div class="sticker"><span>NAKLEJKA W INSTAGRAMIE</span><strong>${escape(r.sticker)}</strong><div><input aria-label="Link do naklejki ${r.number}" readonly value="${escape(r.link)}"><button class="copy-link" data-link="${escape(r.link)}">Kopiuj link</button></div></div><details><summary>Tekst alternatywny</summary><p>${escape(r.alt)}</p></details></div></article>`).join('');
await writeFile(path.join(site,'index.html'),`<!doctype html><html lang="pl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><meta name="description" content="Dziesięć statycznych Stories o pracy nad CHMURNIKIEM z Astrą. Gotowe PNG, pełne teksty i linki do pobrania."><meta name="theme-color" content="#f8e2ea"><title>${escape(campaign.title)} · CHMURNIK</title><link rel="stylesheet" href="gallery.css"></head><body><header><a href="../"><img src="assets/wordmark.png" width="200" alt="CHMURNIK"></a><span>PAKIET / 04.09.2026</span></header><main><section class="intro"><p class="kicker">10 STATYCZNYCH STORIES · PNG 1080 × 1920</p><h1>W tym niebie<br>jest trochę Astry.</h1><p class="lede">Pełna opowieść, miękkie kolory i prawdziwe ekrany aplikacji. Wszystkie dziesięć plansz w kolejności, gotowe do Instagram Stories.</p><a class="button big" href="${zipName}" download>Pobierz wszystkie PNG <span>↓ ZIP · ${(zipSize/1048576).toFixed(1)} MB</span></a><p class="hint">Na telefonie możesz pobierać lub udostępniać pojedyncze obrazy poniżej. Naklejkę z linkiem dodaj w wolnym miejscu nad dolną linią każdej planszy.</p></section><section class="stories">${cards}</section><footer id="credits"><h2>Źródła i szczegóły</h2><p>Ekrany aplikacji są autentyczne. Plansze 06–07: interfejs mobilny uchwycony w izolowanej przeglądarce. Plansza 08: symulator iPada. Plansza 09: natywna aplikacja na Macu, w recenzji Apple według stanu na 4 września 2026. Ramy urządzeń i ilustracje są dekoracyjne. Nie zmieniono danych ani fotografii wewnątrz ekranów.</p><p>Fotografia Cirrus: PiccoloNamek, <a href="https://commons.wikimedia.org/wiki/File:CirrusField-color.jpg">CirrusField-color.jpg</a>, <a href="https://creativecommons.org/licenses/by-sa/3.0/">CC BY-SA 3.0</a>. <a href="ZRODLA-ZDJEC.txt">Pełne informacje o źródłach i użyciu</a>. Obecność fotografii nie oznacza poparcia aplikacji przez autora. Przykłady pogody służą do nauki.</p><p><a href="TEKSTY-I-LINKI.txt">Wszystkie teksty i linki</a> · <a href="CZYTAJ-MNIE.txt">Instrukcja</a> · <a href="../historia/">Poprzednia opowieść</a> · <a href="https://chmurnik.cloud/">chmurnik.cloud</a></p></footer></main><div role="status" id="status" aria-live="polite"></div><script src="gallery.js"></script></body></html>`);
