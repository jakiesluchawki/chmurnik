...
const releasePrefix='https://github.com/jakiesluchawki/chmurnik/releases/download/';
const external=file=>file.startsWith(releasePrefix);
const href=(pack,file)=>external(file)?file:pack.base+file;
for(const pack of packs){
  for(const file of ['index.html',pack.preview,...pack.downloads.map(d=>d[1]).filter(f=>!external(f))])assert((await stat(path.join(social,pack.source,'site',file))).isFile(),`Missing asset: ${pack.source}/${file}`);
}
